...
// Existing products keyed by name (trimmed, lowercased) so re-running an
// import updates the same rows instead of creating duplicates, and so a
// re-run without a new image doesn't clobber one that's already set.
// Paginated because PostgREST caps a single select at 1000 rows — without
// this, catalogs over 1000 products would silently miss matches past the
// first page and duplicate them instead of updating.
async function getExistingProductsByName() {
  const map = new Map();
  const pageSize = 1000;
  let from = 0;
  for (;;) {
    ...
    data.forEach((p) => map.set(p.name.trim().toLowerCase(), p));
...
async function convertShopifyCsv(buffer) {
  const records = parse(buffer, { columns: true, skip_empty_lines: true, trim: true, bom: true });
  const existingByName = await getExistingProductsByName();
  ...
  records.forEach((row, i) => {
    ...
    rows.push({
      ...
      _existingImage: existing ? existing.image : '',
      featured: '',
...
// replaces it with our own Storage URL. If there's no URL to fetch, or the
// fetch fails, it falls back to whatever image the product already had
// (on an update) rather than resetting a real photo back to the generic
// placeholder — that fallback is only used for genuinely new products.
// Runs with limited concurrency since a large catalog can mean thousands
  ...
  await mapWithConcurrency(rows, concurrency, async (row) => {
    ...
    if (sourceUrl) {
      try {
        ...
        notes.push(`${row.name}: couldn't fetch image (${err.message}) — kept the existing image instead`);
        row.image = row._existingImage || '';
      }
    } else if (row._existingImage) {
      row.image = row._existingImage;
    }
    delete row._existingImage;
    done += 1;
